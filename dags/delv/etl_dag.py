...
from etl_delv.extract import DeliveryDataExtractor
from etl_delv.transform import DeliveryDataTransformer
...
dag = DAG(
    'kbu1_ppic_delv_pipeline',
    default_args=default_args,
    description='ETL pipeline untuk PPIC Delivery DELV',
...
# Path Configuration
# ====================
RAW_PATH = "/opt/airflow/sharepoint_defect_tracker/KBU1/PPIC Dept/Excel Online/kbu1_ppic_delv_bronze.xlsx"
BRONZE_PATH = "/opt/airflow/sharepoint_defect_tracker/KBU1/PPIC Dept/Extract Data/kbu1_ppic_delv_bronze.xlsx"
SILVER_PATH = "/opt/airflow/sharepoint_defect_tracker/KBU1/PPIC Dept/Silver Data/kbu1_ppic_delv_silver.xlsx"
TABLE_BRONZE = "kbu1_ppic_delv_bronze"
TABLE_SILVER = "kbu1_ppic_delv_silver"
...
def setup_postgres():
    print("🔧 Setup PostgreSQL untuk data DELV")
    get_postgres_connection()

def extract_delv_data():
    extractor = DeliveryDataExtractor(input_path=RAW_PATH, output_path=BRONZE_PATH)
    extractor.run()
    print(f"✅ Bronze file saved: {BRONZE_PATH}")
    save_excel_to_postgres(BRONZE_PATH, TABLE_BRONZE, 'bronze')

def transform_delv_data():
    transformer = DeliveryDataTransformer(input_path=BRONZE_PATH, output_path=SILVER_PATH)
...
t1 = PythonOperator(
    task_id='extract_delv_data',
    python_callable=extract_delv_data,
    dag=dag,
...
t2 = PythonOperator(
    task_id='transform_delv_data',
    python_callable=transform_delv_data,
    dag=dag,
